                   ...
                   unsigned int transitions)
{
   ...
   /* No global data by default, nor custom error handler */
                             ...
                             const char     *transition)
{
   ...
   if (!tr)
     {
        snprintf(msg, sizeof(msg),
                 "Could not find transition \"%s\" at current state (%s)",
   ...
   if (EINA_UNLIKELY(tr->from != mach->current_state))
     {
        snprintf(msg, sizeof(msg),
                 "Wtf?! Transition \"%s\" is inconsistent", tr->name);
        CRI(msg);
        goto fail;
     }
...
fail:
   if (mach->error_cb)
     mach->error_cb(mach, ESTATE_ERROR_TRANSITION, transition, msg);
...
EAPI void
estate_machine_error_cb_set(Estate_Machine        *mach,
                            const Estate_Error_Cb  error_cb)
{
   EINA_SAFETY_ON_NULL_RETURN(mach);
   mach->error_cb = error_cb;
}
